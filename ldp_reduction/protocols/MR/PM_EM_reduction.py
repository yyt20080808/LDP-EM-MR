# coding=utf-8
import numpy as np
from ..numericalProtocols.PM_EM import EMS
...
def PM_E(noisy_samples, eps, randomized_bins=1024, domain_bins=1024, loglikelihood_threshold=1e-3, ifReduct=False,
         remained=1):
    ...
    while True:
        ...
        if value[minIndex] < throd / (2 ** count_remove):
            ...
            for i in range(length):
                theta[i + minIndex+1] = 0
                indexset[i + minIndex+1] = 1
            n = n - length
            if reports_number > 100000:
                para = 1.1
            else:
...
def EM_reduct(throd, reports_number, ns_hist, transform, max_iteration, loglikelihood_threshold, remained, bin_number):
    ...
    while True:
        ...
        minIndex = value.index(min(value))
        if reports_number < 100000 and abs(value[minIndex] - value[largest_v]) < superF / (2 ** (count_remove)) and \
                value[mini_v] > 2 * value[largest_v]:
            minIndex = largest_v
